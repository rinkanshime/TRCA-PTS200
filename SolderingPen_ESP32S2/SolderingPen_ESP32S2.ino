  ...
  while (repeat) {
    ...
    switch (selection) {
      ...
      case 9: {
        ...
        u8g2.setFont(cyrillic);  // choose a suitable font
        u8g2.drawStr(0, 10,
                     "Обновление прошивки MSC");  // write something to the internal memory
      ...
      case 11: {
              ...
              sizeof(ControlTypeItems), PIDenable);
      } break;
...
uint8_t MenuScreen(const char *Items[][language_types], uint8_t numberOfItems,
  ...
  bool isTipScreen = ((strcmp(Items[0][language], "烙铁头:") == 0) ||
                      ...
                      (strcmp(Items[0][language], "Жало:") == 0));
...
void InfoScreen() {
  ...
  do {
    ...
    do {
      ...
      u8g2.print(VERSION);
      u8g2.setCursor(0, 16 * 3 + SCREEN_OFFSET);
      u8g2.print(txt_message[language]);
      u8g2.print(MESSAGE); 
...
// Экран температурной калибровки
